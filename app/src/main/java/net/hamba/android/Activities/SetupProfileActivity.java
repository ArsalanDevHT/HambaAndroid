...
public class SetupProfileActivity extends BaseActivity implements View.OnClickListener, DatePickerDialog.OnDateSetListener, RadioGroup.OnCheckedChangeListener{
    private LinearLayout form;
    private Button btn_save;
    private TextInputLayout TI_dob;
    private TextInputLayout TI_city;
    private TextInputLayout TI_zipcode;
    private EditText ET_dob,ET_email,ET_fullname,ET_phone,ET_city,ET_zipcode,ET_description;
    private Spinner spn_country,spn_nationality,spn_interests;
    ...
    private void InitUI() {
        ...
        spn_interests   = findViewById(R.id.spn_interests);
        ...
        switch (v.getId()){
            case R.id.btn_save:
                if(FormUtils.isValidTextInputParent(form)){
                    ...
                    model.setDescription(FormUtils.getText(ET_email));
                    ...
                    model.setDescription(FormUtils.getText(ET_description));
                    model.setInterests(spn_interests.getSelectedItem().toString());
                    model.setProfilePercent("40% Complete");
                    database.getReference("users").child(auth.getUid()).setValue(model).addOnCompleteListener(new OnCompleteListener<Void>() {
                        @Override
                        public void onComplete(@NonNull Task<Void> task) {
                            setProgressDialogMessage("Finalizing...");
                            if(task.isSuccessful()) {
                                PrefsUtils.saveFullUser(true);
                                startActivity(new Intent(SetupProfileActivity.this,IndividualHomeActivity.class));
                                finish();
                            }
                        }
                    });
        ...
        database.getReference("users").child(auth.getUid()).addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                if(dataSnapshot.exists()){
                    model = dataSnapshot.getValue(IndividualModel.class);

...
public class EmployeeSetupProfileActivity extends BaseActivity implements View.OnClickListener, DatePickerDialog.OnDateSetListener, RadioGroup.OnCheckedChangeListener{
    private LinearLayout form;
    private Button btn_save;
    private TextInputLayout TI_dob,TI_city,TI_zipcode,TI_description;
    private EditText ET_dob,ET_email,ET_fullname,ET_phone,ET_city,ET_zipcode,id_or_passport;
    private Spinner spn_country,spn_nationality;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_employee_setup_profile);
        InitUI();
        setData();
    }
    private void InitUI() {
        ...
        switch (v.getId()){
            case R.id.btn_save:
                if(FormUtils.isValidTextInputParent(form)){
                    ...
                    model.setDescription(FormUtils.getText(TI_description.getEditText()));
                    model.setProfilePercent("40% Complete");
                    database.getReference("users/shop/employeer")
                            .child(PrefsUtils.getPreSharedKey())
                            .child("employees").child(auth.getUid()).setValue(model).addOnCompleteListener(new OnCompleteListener<Void>() {
                        @Override
                        public void onComplete(@NonNull Task<Void> task) {
                            setProgressDialogMessage("Finalizing...");
                            if(task.isSuccessful()) {
                                PrefsUtils.saveFullUser(true);
                                startActivity(new Intent(EmployeeSetupProfileActivity.this,OwnerHomeActivity.class));
                                finish();
                            }
                        }
                    });
    ...
    public void setData(){
        database.getReference("users/shop/employeer")
                .child(PrefsUtils.getPreSharedKey())
                .child("employees").child(auth.getUid()).addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                Log.e( "onDataChange: ",PrefsUtils.getPreSharedKey());

                if(dataSnapshot.exists()){
                    model = dataSnapshot.getValue(EmployeeModel.class);
